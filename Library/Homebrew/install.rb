    ...
    class << self
      ...
      def print_dry_run_dependencies(formula, dependencies, skip_formula_names: [], &_block)
        ...
        entries = dependencies.filter_map do |dep|
          ...
          reinstall = installed && dependency.outdated? && dependency.latest_version_installed?
          name = "#{name} (reinstall)" if reinstall
          [installed, name]
        end

        to_update, to_install = entries.partition(&:first)
        { "install" => to_install, "update" => to_update }.each do |verb, group|
          next if group.empty?
